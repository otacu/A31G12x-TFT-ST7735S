...
static void uart_set_divisors( UARTn_Type* UARTx, uint32_t baudrate )
{
   ...
   //--------------------------------------
   // bdr = UARTn_BaseClock / (16 * baudrate * 2)
   //--------------------------------------
   ...
   // bfr = (numerator / denominator - bdr) * 256
   //     = (numerator - bdr * denominator) * 256 / denominator
   //     = fd * 256 / denominator
   //--------------------------------------
   fd = numerator - bdr * denominator;
...
HAL_Status_Type HAL_UART_Init( UARTn_Type* UARTx, UARTn_CFG_Type* UARTn_Config )
{
   ...
   if( UARTx == ( UARTn_Type* )UART0 )
   {
      ...
      HAL_SCU_Peripheral_EnableClock2( PPCLKEN2_UT0CLKE, PPxCLKE_Enable );
   }
#endif
